...
from skeleton.config import PG_DSN, NEO4J_URI
...
from skeleton.exceptions import TransitFlowException
...
def error_handler(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            return json.dumps({"success": False, "error": {"message": e.message, "code": e.error_code}})
        except Exception:
            logging.error("INTERNAL_ERROR in %s:\n%s", func.__name__, traceback.format_exc())
            return json.dumps({"success": False, "error": {"message": "An internal error occurred.", "code": "INTERNAL_ERROR"}})
    return wrapper
...
class TransitFlowAgent:
    """
    Orchestrates the full LLM ↔ database pipeline.

    All database access goes through the injected service objects — the agent
    never calls database drivers directly.
    """

    def __init__(
        self,
        db_service: RelationalService,
    ...
    def _execute_tool(
        ...
        _status = "success"
        try:
            return self._execute_tool_inner(tool_name, params, current_user_email)
        except Exception:
            _status = "error"
            raise
        finally:
            ...
            _logger.info(
                "tool_executed",
                tool=tool_name,
                duration_ms=round(_elapsed * 1000, 2),
                status=_status,
            )
            try:
                from skeleton.metrics import query_counter, query_duration
                query_counter.labels(tool=tool_name, status=_status).inc()
                query_duration.labels(tool=tool_name).observe(_elapsed)
            except ImportError:
                pass
    ...
    def _execute_tool_inner(
        ...
        if tool_name == "check_national_rail_availability":
            result = self.db.query_national_rail_availability(**params)

        elif tool_name == "get_national_rail_fare":
            result = self.db.query_national_rail_fare(**params)

        elif tool_name == "check_metro_availability":
            result = self.db.query_metro_schedules(
                origin_id=params["origin_id"],
                destination_id=params["destination_id"],
            )

        elif tool_name == "calculate_metro_fare":
            result = self.db.query_metro_fare(**params)

        elif tool_name == "get_metro_fare":
            schedules = self.db.query_metro_schedules(
                origin_id=params["origin_id"],
                destination_id=params["destination_id"],
            )
            if not schedules:
                ...
                try:
                    ...
                    n_stops = 1
                fare = self.db.query_metro_fare(sched["schedule_id"], n_stops)
                result = {
            ...
            result = self.db.query_user_bookings(current_user_email)

        elif tool_name == "get_available_seats":
            result = self.db.query_available_seats(**params)

        elif tool_name == "make_booking":
            if not current_user_email:
                return json.dumps({"error": "You must be logged in to make a booking."})
            profile = self.db.query_user_profile(current_user_email)
            if not profile:
                return json.dumps({"error": "User profile not found."})
            ok, data = self.db.execute_booking(
            ...
            if not profile:
                return json.dumps({"error": "User profile not found."})
            ok, data = self.db.execute_cancellation(
            ...
            docs = self.db.query_policy_vector_search(embedding)
            result = [
                {
                    "title":      d["title"],
                    "category":   d["category"],
            ...
            routes = self.graph.query_alternative_routes(
                origin_id=params["origin_id"],
                destination_id=params["destination_id"],
            ...
            result = self.graph.query_delay_ripple(
...
JSON:"""
        ...
        for call in tool_calls:
            ...
            result_json = self._execute_tool(tool_name, params, current_user_email)
...
# ── Module-level default instance (backward compatibility) ────────────────────

_default_agent = TransitFlowAgent(
    db_service=PostgreSQLService(PG_DSN),
    graph_service=Neo4jService(NEO4J_URI),
)
...
def run_agent(
    ...
    """Module-level convenience wrapper — delegates to the default agent instance."""
    return _default_agent.run(user_message, history, debug, current_user_email, progress_callback)
